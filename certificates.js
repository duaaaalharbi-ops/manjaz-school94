window.MANJAZ_CERTIFICATES_RUNTIME_VERSION = "10.7.3-visual-reference-match";
(function(){
  "use strict";
  ...
  function injectStyle(){
    ...
    link.rel="stylesheet"; link.href="certificates.css?v=10.7.3"; link.dataset.certificatesStyle="1";
  ...
  function implementersLabel(w){
    ...
    return `${names.length>1?"منفذات الورشة":"منفذة الورشة"}: ${names.map(esc).join("، ")}`;
  }
  ...
  function renderCertificate(w,name){
    ...
    paper.className="certificate-paper cert-approved cert-visual-reference";
    paper.style.backgroundImage="";
    paper.innerHTML=`
      <div class="cert-reference-bg" aria-hidden="true">
        <span class="cert-curve cert-curve-tr"></span>
        <span class="cert-curve cert-curve-bl"></span>
        <span class="cert-leaves cert-leaves-r"></span>
        <span class="cert-leaves cert-leaves-l"></span>
      </div>

      <header class="cert-reference-header">
      ...
      <main class="cert-reference-main">
        <h1 class="cert-reference-title">شهادة حضور</h1>
        <div class="cert-reference-program">ورشة «${workshopTitle}»</div>

        <div class="cert-reference-testimony">تشهد الثانوية الرابعة والتسعون - مسارات بأن المعلمة</div>
        <div class="cert-reference-name">${esc(name)}</div>

        <div class="cert-reference-attendance">
          قد حضرت الورشة التدريبية بعنوان «${workshopTitle}»، والتي نُفذت يوم ${date}، بواقع ${durationPhrase}
        </div>
        <div class="cert-reference-wish">متمنين لها دوام التوفيق ومزيدًا من التميز والعطاء</div>

        ${implementersBlock(w)}
      </main>

      <footer class="cert-reference-footer">
