window.MANJAZ_CERTIFICATES_RUNTIME_VERSION = "10.7.22-kashouf-workshop";
(function(){
  "use strict";
  ...
  function injectStyle(){
    ...
    link.rel="stylesheet"; link.href="certificates.css?v=10.7.22"; link.dataset.certificatesStyle="1";
  ...
  function wirePage(){
    ...
    document.querySelectorAll(".cert-choose").forEach(btn=>btn.addEventListener("click",()=>{
      ...
      hiddenId.value=w.id;
      const sd=normalizeWorkshopDate(w.date); selected.innerHTML=`<strong>${esc(w.title)}</strong><span>${esc(sd.day)} ${dateSpan(sd.date)} • ${esc(workshopDurationLabel(w))}</span>${implementersLabel(w)?`<small>${implementersLabel(w)}</small>`:""}`;
