(function(){
  ...
  function injectHomeCard(){
    ...
    const grid=document.querySelector("#view .category-grid");
    if(!grid || grid.querySelector(".cert-home-card")) return;
    const card=document.createElement("article");
    card.className="category-card cert-home-card";
    card.innerHTML='<span class="cat-no">09</span><strong>إصدار شهادات الورش المنفذة الرقمية</strong><small>أنشئي شهادتك المعتمدة إلكترونيًا للورش المنفذة دون حفظ بياناتك</small>';
    card.addEventListener("click",()=>location.hash="#certificates");
  ...
  window.addEventListener("hashchange",()=>{
    setTimeout(()=>{
      if((location.hash||"").slice(1)===ROUTE) renderPage(); else injectHomeCard();
    },0);
  });
