window.MANJAZ_CERTIFICATES_RUNTIME_VERSION = "10.7.17-final-certificate-rule";
(function(){
  "use strict";
  ...
  function injectStyle(){
    ...
    link.rel="stylesheet"; link.href="certificates.css?v=10.7.15"; link.dataset.certificatesStyle="1";
  ...
  async function snapshotCurrentPreview(){
    ...
    // Important: export the CURRENT preview DOM itself, never rebuild the certificate.
    // The clone is only moved off-screen and unscaled so mobile CSS transform cannot
    // change the captured geometry. No text, position, logo, stamp or layout is changed.
    ...
    clone.style.setProperty("width","1123px","important");
    clone.style.setProperty("height","794px","important");
    ...
    try{
      ...
      // Two animation frames ensure the exact cloned preview has completed layout/paint.
  ...
  async function downloadPDF(w,name){
    ...
    try{
      btn.disabled=true;msg.className="cert-message info";msg.textContent="جارٍ إنشاء الشهادة المطابقة للمعاينة...";
      await injectLibraries();

      // The PDF is a lossless snapshot of the same approved preview DOM.
      const canvas=await snapshotCurrentPreview();
      const img=canvas.toDataURL("image/png");

      const {jsPDF}=window.jspdf;
      const pdf=new jsPDF({orientation:"landscape",unit:"mm",format:"a4",compress:true});
      pdf.addImage(img,"PNG",0,0,297,210,undefined,"NONE");
      pdf.save(fileName(w,name));

      msg.className="cert-message success";
      msg.innerHTML='<strong>تم إنشاء الشهادة بنجاح</strong><br>الإصدار مطابق للمعاينة الحالية، دون حفظ الاسم أو الشهادة في الموقع';
    }catch(err){
      console.error(err);
