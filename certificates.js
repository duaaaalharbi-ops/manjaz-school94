...
(function(){
  ...
  function renderCertificate(w,name){
    ...
    const multi=implementers.length>1;
    const today=new Intl.DateTimeFormat("ar-SA-u-ca-islamic",{day:"numeric",month:"numeric",year:"numeric"}).format(new Date());
    const workshopTitle=esc(w.title||"");
    const duration=esc(w.duration||"");
    const date=esc(w.date||"");
    const bodyText=`قد حضرت ورشة العمل بعنوان «${workshopTitle}»، والمنفذة يوم ${date}، بواقع ${duration} تدريبيتين، متمنين لها دوام التوفيق ومزيدًا من التميز والعطاء`;

    paper.className="certificate-paper cert-professional";
    paper.style.backgroundImage="";
    paper.innerHTML=`
      <div class="cert-default-bg"></div>
      <i class="cert-accent a"></i><i class="cert-accent b"></i>

      <header class="cert-letterhead">
        <div class="cert-letterhead-right">
          <strong>وزارة التعليم</strong>
          <span>إدارة تعليم جدة</span>
          <span>الثانوية الرابعة والتسعون - مسارات</span>
        </div>
        <div class="cert-ministry-mark" aria-label="شعار وزارة التعليم">
          <div class="cert-ministry-dots" aria-hidden="true">
            <i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i>
          </div>
          <strong>وزارة التعليم</strong>
          <small>Ministry of Education</small>
        </div>
        <div class="cert-letterhead-left">
      ...
      <main class="cert-main">
        ...
        <div class="cert-program">ورشة «${workshopTitle}»</div>

        <div class="cert-testimony">تشهد الثانوية الرابعة والتسعون - مسارات بأن المعلمة</div>
        <div class="cert-beneficiary-name">${esc(name)}</div>
        <div class="cert-text">${bodyText}</div>

        <div class="cert-workshop-details">
          <span><b>التاريخ:</b> ${date}</span>
          <span><b>المدة:</b> ${duration}</span>
          ${implementers.length?`<span class="cert-detail-implementers"><b>${multi?'منفذات الورشة':'منفذة الورشة'}:</b> ${implementers.map(esc).join('، ')}</span>`:""}
        </div>
      </main>
      ...
      <footer class="cert-footer">
        ...
        <div class="cert-footer-principal">
      ...
      <div class="cert-watermark">تم إصدار هذه الشهادة إلكترونيًا عبر بوابة منجز الرقمية</div>`;
  }
